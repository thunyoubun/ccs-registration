...
function RowData({ date }: { date: IProgrammeData[] }) {
    ...
    return date.map((schedule, index) =>
        <React.Fragment key={schedule.time + index}>
            <tr key={schedule.topic + index} className={`${schedule.bgColor ? "bg-rose-700 shadow rounded-lg text-white" : " hover:bg-gray-100 text-black"} shadow rounded-lg`}>
...
function TableProgramme() {
    ...
    return (
        <div className="bg-white grid-cols-1 rounded shadow-lg overflow-auto">
            <div className="rounded-t-sm bg-rose-700 p-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-6">
                <button
                    className={`${!selectSecond ? "bg-white text-rose-700 rounded shadow-lg" : "text-black"} p-3 rounded-lg flex justify-center text-center col-span-3`}
